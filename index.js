...
var pacResolver = require('../node-pac-resolver');
...
function connect(req, opts, fn) {
    ...
    function onproxy(proxy) {
        ...
        case 'HTTPS':
            ...
            proxyURL = ('HTTPS' === type ? 'https' : 'http') + '://' + parts[1];
            proxy = extend({}, self.proxy, parse(proxyURL));
            if (secure) {
    ...
    function onresolver(err, FindProxyForURL) {
        ...
        FindProxyForURL(url, host).then(onproxy).catch(onerror);
    }
...
function PacProxyAgent(uri, opts) {
    ...
    if (!(this instanceof PacProxyAgent)) {
        return new PacProxyAgent(uri, opts);
    }
    ...
    if (!uri) {
        throw new Error('a PAC file URI must be specified!');
    }
...
PacProxyAgent.prototype.loadResolver = function (fn) {
    ...
    function onpacfile(err, code) {
        ...
        self._resolver = pacResolver(code, {
            filename: self.uri,
